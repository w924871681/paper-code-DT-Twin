# -*- coding: utf-8 -*-
"""Independent publication plots for current-manuscript Fig. 6--12."""
...
def plot_fig6(data_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    for ax, reference_key, proposed_key, title in (
        (axes[0], "pt_ft_wmse", "proposed_wmse", "(a) MSE"),
...
def plot_fig7(data_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    fig.subplots_adjust(left=0.105, right=0.985, top=0.91, bottom=0.24, wspace=0.48)
    for ax, panel, title, color in (
        (axes[0], "H/K", "(a) Horizon and support size", "#0072B2"),
        (axes[1], "Center type", "(b) Target-center type", "#D55E00"),
    ):
        ...
        ax.set_xlabel("Mean paired MSE reduction (%)")
        ax.set_title(title, loc="left")
    ...
    fig.text(
        0.5,
        0.06,
        "Points are means; error bars are 95% center-cluster bootstrap confidence intervals.",
        ha="center",
        fontsize=6.4,
    )
...
def plot_fig11(data_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    offsets = {
        "MLP3-32": (0.045, 1.15),
        "MLP4-32": (0.045, -1.05),
        "Alt GRU16": (0.055, 1.15),
        "Alt GRU32": (-0.255, 1.30),
        "Ref GRU32": (-0.235, 1.15),
    }
...
def plot_fig12(data_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    ax_a.text(20.7, 8.6, "Main setting", rotation=90, va="top", fontsize=6.2)
