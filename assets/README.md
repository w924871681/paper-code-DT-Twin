...
Git history. Release `v1.1.7` publishes:

- `level_c_bootstrap_v1.1.7.zip` - 32 frozen files required by the locked
  main-evaluation preflight;
- `cuda_replay_evidence_v1.1.7.zip` - sanitized completed replay evidence;
- `paper_alignment_v1.1.7.zip` - regenerated tables, figures, plot-ready data,
  and validation manifests;
- `rcf_dti_v1.1.7_complete.zip` - manuscript, figures, code, data, configs,
  documentation, and audit evidence;
- `RCF_DTI_FIGURE_CODE_FINAL_V1_1_7.zip` - standalone final plotting package;
- sidecars and the combined `SHA256SUMS.txt`.

The Level-C and CUDA-evidence ZIP payloads are byte-identical to the verified
v1.1.4-v1.1.6 payloads; only their v1.1.7 release filenames change:
...
python .\scripts\verify_release_evidence.py `
  .\cuda_replay_evidence_v1.1.7.zip
```
